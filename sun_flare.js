  ...
  function Flare() {
    ...
    this.draw = function (obj) {
      ...
      var dist =
        1 -
        (0.5 *
          Math.sqrt(Math.pow(obj.x - mid_x, 2) + Math.pow(obj.y - mid_y, 2))) /
      ...
      for (var i = 0; i < this.discs.length; i++) {
        ...
        if (i == 0) {
          ...
          grad.addColorStop(
            0.6,
            "hsla(" + this.discs[i].hue + ",100%,75%," + 0.3 * sdist + ")"
          );
    ...
    this.update = function (obj) {
      ...
      for (var i = 0; i <= this.discNum; i++) {
        ...
        if (this.t == 0) {
          temp.dia =
            Math.pow(Math.abs(7 * (j / this.discNum)), 2) * 3 +
            (Math.random() * 20 + 10);
