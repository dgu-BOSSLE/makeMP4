var rays = new Object({
  ...
  update: function () {
    ...
    for (i = 0; i < rays.num_rays; i++) {
      this.opacitys[i] =
        Math.abs(Math.sin(rays.offset + this.offsets[i])) * 0.8 + 0.2; // Random starting offset for each ray
    }
  ...
  resetCanvas: function () {
    ...
    rays.diameter = Math.sqrt(
      Math.pow(rays.canvas.width, 2) + Math.pow(rays.canvas.height, 2)
    );
...
function combineCanvases() {
  var canvasRay = document.getElementById("ray");
  var canvasFlare = document.getElementById("flare");
  var canvasCombined = document.getElementById("combined");

  var contextCombined = canvasCombined.getContext("2d");

  contextCombined.clearRect(0, 0, canvasCombined.width, canvasCombined.height);
  contextCombined.drawImage(canvasRay, 0, 0);
  contextCombined.drawImage(canvasFlare, 0, 0);
}
